	...
	public function __construct($array)
	{
		if (isset($array['dest']) && $array['dest'] instanceof Vertex)
		{
			if (isset($array['orig']) && $array['orig'] instanceof Vertex)
				$instanceOrig = new Vertex(array('x' => $array['orig']->getX(), 'y' => $array['orig']->getY(), 'z' => $array['orig']->getZ()));
			else
				$instanceOrig = new Vertex(array('x' => 0, 'y' => '0', 'z' => 0));
			$this->_x = $array['dest']->getX() - $instanceOrig->getX();
			$this->_y = $array['dest']->getY() - $instanceOrig->getY();
			$this->_z = $array['dest']->getZ() - $instanceOrig->getZ();
			$this->_w = 0;
		}
		if (Self::$verbose)
